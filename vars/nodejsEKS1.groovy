    ...
    pipeline {
        ...
        stages {
            ...
            stage('verify Deployment'){
                steps{
                    script{
                        rollbackStatus = sh(script: "kubectl rollout status deployment/frontend -n ${project} --timeout=1m || true", returnStdout: true).trim()
                        if(rollbackStatus.contains('successfully rolled out')){
                            ...
                            if(releaseExists.isEmpty()){
                                ...
                                sh """
                                    aws eks update-kubeconfig --region ${region} --name ${project}-dev
                                    helm rollback frontend -n ${project} 0
                                    sleep 60
                                """
                                rollbackStatus = sh(script: "kubectl rollout status deployment/frontend -n expense --timeout=2m || true", returnStdout: true).trim()
